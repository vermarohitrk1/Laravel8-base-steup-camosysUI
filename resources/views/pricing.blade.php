            ...
            <div class="content-body">
                <section id="pricing-plan">
                    ...
                    <div class="tab-content plans">
                        <div class="tab-pane active" id="monthly" aria-labelledby="home-tab" role="tabpanel">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="annual-plan">
                                                        <div class="plan-price mt-2">
                                                            ...
                                                            <span class="pricing-basic-value font-weight-bolder text-primary">{{$month->total_amount}}</span>
                                                    ...
                                                    <ul class="list-group list-group-circle text-left">
                                                        @foreach($month->plansfeatures as $features)
                                                        <li class="list-group-item">{{$features->title}}</li>
                                                        @endforeach
                                                    </ul>
                                                    @if($month->type == 'Custom')
                                                        <button class="btn btn-block btn-primary mt-2 waves-effect fetch-display-click" type="button" data="id:{{$month->id}}|_token:{{ csrf_token() }}" url="{{url('/plan/customize')}}" href="javascript:void(0);" holder=".update-holder" modal="#customize">
                                                        <i data-feather='edit'></i> Customize </button> 
                                                        <a class="fetch-display-click" >
                                                        </a>
                                                        @else                                                   
                                                    <button onclick="location.href = '/checkout/{{$month->id}}';" class="btn btn-block btn-primary mt-2">Buy</button>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div class="tab-pane" id="annually" aria-labelledby="home-tab" role="tabpanel">
                        <div class="row pricing-card">
                                <div class="col-12 col-sm-offset-2 col-sm-10 col-md-12 col-lg-offset-2 col-lg-10 mx-auto">
                                    <div class="row">
                                    @foreach($annually as $annual)
                                            ...
                                            <div class="card basic-pricing text-center">
                                                <div class="card-body">
                                                    ...
                                                    <div class="annual-plan">
                                                        <div class="plan-price mt-2">
                                                            <sup class="font-medium-1 font-weight-bold text-primary">$</sup>
                                                            <span class="pricing-basic-value font-weight-bolder text-primary">{{$annual->total_amount}}</span>
                                                            <sub class="pricing-duration text-body font-medium-1 font-weight-bold">/year</sub>
                                                        </div>
                                                        <small class="annual-pricing d-none text-muted"></small>
                                                    </div>
                                                    <ul class="list-group list-group-circle text-left">
                                                        @foreach($annual->plansfeatures as $features)
                                                        <li class="list-group-item">{{$features->title}}</li>
                                                        @endforeach
                                                    </ul>
                                                    @if($annual->type == 'Custom')
                                                        <button class="btn btn-block btn-primary mt-2 waves-effect fetch-display-click" type="button" data="id:{{$annual->id}}|_token:{{ csrf_token() }}" url="{{url('/plan/customize')}}" href="javascript:void(0);" holder=".update-holder" modal="#customize">
                                                        <i data-feather='edit'></i> Customize </button> 
                                                        <a class="fetch-display-click" >
                                                        </a>
                                                        @else                                                   
                                                    <button onclick="location.href = '/checkout/{{$annual->id}}';" class="btn btn-block btn-primary mt-2">Buy</button>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
...
<script src="{{ asset('app-assets/js/scripts/forms/form-repeater.js') }}"></script>
 ...
 <script>
 $(document).on('change','input:checkbox#priceSwitch',function(){
     if($(this).is(':checked')){
        $('.plans .tab-pane').removeClass('active');
        $('.plans .tab-pane#annually').addClass('active');
     }else
     {
        $('.plans .tab-pane').removeClass('active');
        $('.plans .tab-pane#monthly').addClass('active');
     }
 });

 </script>
     <!-- modals -->
<div class="modal fade text-left" id="customize" tabindex="-1" aria-labelledby="myModalLabel1" style="display: none; padding-right: 17px;" aria-modal="true" role="dialog">
